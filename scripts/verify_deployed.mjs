/** Read-only acceptance of the existing AutoBott deployment. No broker writes. */
...
export function inventory(rows) {
  ...
  for (const row of rows) {
    const symbol = String(row.symbol || row.option_symbol || '').toUpperCase();
...
export async function verify({ token, expected, request = fetch }) {
  ...
  for (const path of ['/api/safety', '/api/v2/pairs', '/api/session/status', '/api/trading/timeline']) {
    const rejected = await get(path, null, 401);
    assert.deepEqual(rejected, { ok: false, error: 'unauthorized' }, 'generic_unauthorized_required');
  }
  ...
  return { verifiedAt: new Date().toISOString(), sourceSha: expected, releaseVerified: true, scope: 'GET-only deployed revision, health, authentication, paper controls and account/pair reads. Not strategy-profitability or complete historical-accounting certification.', checks, paperOnly: true, realMoneyEnabled: false, executionEnabled: safety.execution_enabled, orderPlacementEnabled: safety.order_placement_enabled, killSwitchEnabled: safety.kill_switch_enabled, inventory: { brokerRows: account.positions.length, storedRows: stored.positions.length, snapshotStable: stable, quantityDifferences: differences, storedDuplicateSymbols: storedInventory.duplicateSymbols }, pairs: { count: pairs.pair_count, complete: pairs.open_pair_count, retainedRunners: pairs.retained_runner_count }, session: safeFlags(session), sessionShape: Object.keys(session.state || {}), timelineShape: Object.keys(timeline), accountingShape: Object.keys(learning), accounting: safeFlags(learning), warnings: (timeline.warnings || []).map(row => typeof row === 'object' ? row.type : 'unstructured_warning').slice(0, 20), brokerWrites: 0 };
}
...
if (process.env.AUTOBOTT_VERIFY_DEPLOYED === 'true' && !process.env.NODE_TEST_CONTEXT) {
  ...
  // Do not inherit a production dashboard credential or preload into test workers.
  ...
  try {
    ...
    process.exitCode = 1;
    throw new Error('AutoBott read-only deployed acceptance failed; see private build log');
  }
}
