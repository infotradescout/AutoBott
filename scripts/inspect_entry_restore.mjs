...
import difflib, hashlib, json, pathlib, subprocess, sys, tempfile
...
def deny(event,args):
    if event.startswith('socket.') or event in {'urllib.Request','http.client.connect'}:
        raise RuntimeError('source_inspection_network_forbidden')
sys.addaudithook(deny)
def git(*args, check=True):
    return subprocess.run(['git',*args],check=check,capture_output=True,timeout=30)
def data(ref,path):
    ...
    if donor is None:
        ...
        with tempfile.TemporaryDirectory(prefix='entry-merge-inspect-') as d:
            ...
            if merged.returncode==0:
                ...
                diff=''.join(difflib.unified_diff(current.decode().splitlines(True),merged.stdout.decode().splitlines(True),fromfile=destination,tofile=destination))
                row['patch']=diff
            else:
                row['status']='conflict'
...
print('AUTOBOTT_ENTRY_RESTORE_SUMMARY '+json.dumps({'base':BASE,'donor':DONOR,'current':CURRENT,'files':len(rows),'counts':{s:sum(r['status']==s for r in rows) for s in sorted({r['status'] for r in rows})},'source_modified':False,'network_allowed':False}),flush=True)
`;
  const child = spawnSync('python',['-c',script],{env:safe,encoding:'utf8',timeout:60000,maxBuffer:500000});
