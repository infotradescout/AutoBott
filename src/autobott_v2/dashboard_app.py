        ...
        "positions": [
            {
                ...
                **_option_symbol_parts(str(position.get("symbol") or "")),
                ...
                "real_money_affordable": (
                    stored_by_symbol.get(str(position.get("symbol") or "").upper()).real_money_affordable
                    if stored_by_symbol.get(str(position.get("symbol") or "").upper()) is not None
                    else None
                ),
                "affordable_leg_cost": (
                    stored_by_symbol.get(str(position.get("symbol") or "").upper()).affordable_leg_cost
                    if stored_by_symbol.get(str(position.get("symbol") or "").upper()) is not None
                    else None
                ),
    ...
    function renderAccountPositions(payload) {
      ...
      const rows = positions.map((position) => {
        ...
        return `<tr>
          ...
          <td><span class="badge ${position.real_money_affordable === true ? 'safe' : position.real_money_affordable === false ? 'warn' : 'info'}">${position.real_money_affordable === true ? '≤ $100/leg' : position.real_money_affordable === false ? 'paper only' : 'legacy'}</span></td>
      ...
      return `
        <table class="table">
          <thead><tr><th>Symbol</th><th>Leg</th><th>Affordability</th><th>Type</th><th>Strike</th><th>Side</th><th>Qty</th><th>Entry</th><th>Current</th><th>Unrealized P/L</th></tr></thead>
          <tbody>${rows || '<tr><td colspan="10">No open positions</td></tr>'}</tbody>
        </table>
        ${detailsBlock(payload)}`;
    }
